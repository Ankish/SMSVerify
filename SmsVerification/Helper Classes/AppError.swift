...
public class AppError: Error {
    ...
    static func error(errorMessage: String) -> AppError {
        return AppError(errorTitle: "Error", errorMessage: errorMessage)
    }
    
    static func error(errorType: ErrorType) -> AppError {
        return AppError(errorTitle: "Error", errorMessage: errorType.rawValue)
    }
}
